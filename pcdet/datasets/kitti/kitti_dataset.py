...
class KittiDataset(DatasetTemplate):
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('dair_v2x_dbinfos_%s.pkl' % split)
    ...
    def __getitem__(self, index):
        ...
        # --- 4. 计算标定矩阵 (新增核心部分) ---
        # 兼容两种拼写：calib_matrices (我们的配置) 和 calib_matricies (OpenPCDet旧代码)
        if "calib_matrices" in get_item_list or "calib_matricies" in get_item_list:
            ...
            # 构造 4x4 的 V2C (雷达转相机外参)
            V2C = np.eye(4, dtype=np.float32)
            V2C[:3, :4] = calib.V2C
            
            # 计算总变换矩阵: LiDAR -> Image
            # 公式: Image = P2 * R0 * V2C * Lidar
            trans_lidar_to_img = P2 @ R0 @ V2C
            
            input_dict['trans_lidar_to_img'] = trans_lidar_to_img
            # 同时也存下中间矩阵，以防万一
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    train_filename = save_path / ('dair_v2x_infos_%s.pkl' % train_split)
    val_filename = save_path / ('dair_v2x_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'dair_v2x_infos_trainval.pkl'
    test_filename = save_path / 'dair_v2x_infos_test.pkl'
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        create_kitti_infos(
            ...
            data_path=ROOT_DIR / 'data' / 'dair_v2x',
            save_path=ROOT_DIR / 'data' / 'dair_v2x'
        )
