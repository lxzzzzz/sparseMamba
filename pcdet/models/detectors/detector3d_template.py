...
# 添加魔法映射，欺骗 torch.load 的反序列化器
sys.modules['numpy._core'] = sys.modules['numpy.core']
sys.modules['numpy._core.multiarray'] = sys.modules['numpy.core.multiarray']


def _load_checkpoint_with_compat(filename, map_location=None, logger=None):
    try:
        ...
        if logger is not None:
            logger.warning(
                'torch.load hit the PyTorch 2.6 weights_only default when reading %s; '
...
class Detector3DTemplate(nn.Module):
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False, pre_trained_path=None):
        ...
        checkpoint = _load_checkpoint_with_compat(filename, map_location=loc_type, logger=logger)
        model_state_disk = checkpoint['model_state']
        if not pre_trained_path is None:
            pretrain_checkpoint = _load_checkpoint_with_compat(
                pre_trained_path, map_location=loc_type, logger=logger
            )
        ...
        if optimizer is not None:
            if 'optimizer_state' in checkpoint and checkpoint['optimizer_state'] is not None:
                ...
                if os.path.exists(optimizer_filename):
                    optimizer_ckpt = _load_checkpoint_with_compat(
                        optimizer_filename, map_location=loc_type, logger=logger
                    )
