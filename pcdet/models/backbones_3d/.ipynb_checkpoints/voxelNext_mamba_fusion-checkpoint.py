...
try:
    ...
    Mamba = None
    print("Warning: mamba_ssm not installed. VoxelNeXtMambaFusion will fallback to Identity.")
...
class DeformableRectifier(nn.Module):
    def __init__(self, dim_lidar, dim_img, stride=8):
        super().__init__()
        self.stride = stride 
        self.num_heads = 4
        self.scale = (dim_lidar // self.num_heads) ** -0.5
        
        self.kernel_offsets = torch.tensor([
    ...
    def forward(self, x_lidar, indices, img_feats, batch_dict):
        ...
        delta_u, delta_v = 2.0 / W, 2.0 / H
        offsets = self.kernel_offsets.to(x_lidar.device)
        offsets_u = offsets[:, 0] * delta_u
        offsets_v = offsets[:, 1] * delta_v
        
        grid = centers_uv.unsqueeze(1) + torch.stack([offsets_u, offsets_v], dim=-1).unsqueeze(0)
        
        sampled_list = []
        for b in range(B):
            mask = (batch_inds == b)
            if mask.sum() == 0: continue
            grid_b = grid[mask].view(1, -1, 17, 2)
            feat_b = F.grid_sample(img_feats[b:b+1], grid_b, align_corners=True, padding_mode='zeros')
            sampled_list.append(feat_b.squeeze(0).permute(1, 2, 0))
            
        if len(sampled_list) > 0:
            sampled_feats = torch.cat(sampled_list, dim=0)
        else:
            sampled_feats = torch.zeros(x_lidar.shape[0], 17, C, device=x_lidar.device)
...
class FusionMambaBlock(nn.Module):
    def __init__(self, d_model, stride=8):
        super().__init__()
        self.rectifier = DeformableRectifier(d_model, d_model, stride=stride)
    ...
    def forward(self, x_sparse, batch_dict, img_feats):
        ...
        x_rect = self.rectifier(x, indices, img_feats, batch_dict)
        p_obj, scale = self.gate(x)
        x_in = self.fusion_proj(torch.cat([x, x_rect], dim=-1))
        x_in = x_in * scale 
        x_mamba = self.mamba(x_in.unsqueeze(0)).squeeze(0)
        x_out = self.norm(x + x_mamba)
...
# =====================================================================
# 3. 瀹岀編缁撳悎浣擄細鍘熸眮鍘熷懗鐨?VoxelNeXt + 浣犵殑 Mamba 铻嶅悎
# =====================================================================
class VoxelNeXtMambaFusion(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        if self.use_fusion:
            ...
            print(f"[VoxelNeXtMambaFusion] Mamba enabled: {Mamba is not None}, freeze image BN: {freeze_img_bn}")
        ...
        # 锟斤拷 鍦?Conv4 (Stride=8) 澶勬敞鍏ヤ綘鐨?Mamba 铻嶅悎妯″潡
        num_blocks = model_cfg.get('NUM_BLOCKS', 3)
        self.mamba_blocks = nn.ModuleList([
            FusionMambaBlock(d_model=channels[3], stride=8) for _ in range(num_blocks)
        ])

        # 淇濈暀瀹樻柟鐨?Conv5 鍜?Conv6 (鏋勫缓瀹忚鎰熷彈閲?
    ...
    def forward(self, batch_dict):
        ...
        # 锟斤拷 鍦ㄦ鎵ц澶氭ā鎬佽瀺鍚堬紒(姝ゆ椂姝ｅソ鏄?Stride=8 鐨勭壒寰?
        if self.use_fusion:
