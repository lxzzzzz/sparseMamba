...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Create tracking infos for the V2X-Xian dataset whose labels store 3D boxes in lidar coordinates'
    )
    parser.add_argument('--data_path', type=str, required=True, help='dataset root, e.g. data/v2x_xian')
    parser.add_argument('--save_path', type=str, default=None, help='output directory, defaults to data_path')
    parser.add_argument('--splits', nargs='+', default=['train', 'val', 'trainval', 'all'])
    parser.add_argument(
        '--format_cfg',
        type=str,
        default='tools/cfgs/dataset_configs/tracking_info_builder/v2x_xian_sequence_lidar.yaml',
        help='dataset layout config for indexing lidar/image/calib files',
    )
    parser.add_argument(
        '--with_score',
        action='store_true',
        help='set if each label row has an extra score field appended after yaw',
    )
...
def parse_sequence_lidar_tracking_labels(label_file, with_score=False):
    ...
    with open(label_file, 'r') as f:
        for raw_line in f:
            ...
            gt_boxes_lidar = np.asarray([float(x) for x in fields[10:17]], dtype=np.float32)
            score = float(fields[17]) if with_score and len(fields) > 17 else -1.0

            frame_to_annos[frame_idx].append({
                'track_id': track_id,
                'name': cls_name,
                'bbox': np.asarray([float(x) for x in fields[6:10]], dtype=np.float32),
...
def build_annos_from_lidar_labels(frame_annos):
    ...
    return {
        ...
        # Downstream tracking code reads gt_boxes_lidar directly. Keep legacy keys populated for compatibility.
        'rotation_y': gt_boxes_lidar[:, 6].astype(np.float32),
...
def get_sequence_lidar_label_cache(root_path, sequence_id, format_cfg, cache, with_score):
    ...
    label_file = resolve_file(root_path, [str(Path(candidate) / f'{sequence_id}.txt') for candidate in label_dir_candidates])
...
def collect_infos_for_split(root_path, split, format_cfg, args):
    ...
    for sequence_id in sorted(grouped_entries.keys()):
        ...
        labels_by_frame = get_sequence_lidar_label_cache(root_path, sequence_id, format_cfg, cache, args.with_score)
        ...
        for fallback_idx, frame_id in enumerate(frame_ids):
            ...
            image_shape = np.asarray(io.imread(image_file).shape[:2], dtype=np.int32)
            ...
            annos = build_annos_from_lidar_labels(labels_by_frame.get(frame_idx, []))
